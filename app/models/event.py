...
class Event(Base):
    ...
    # Если задан — подставляется между «Состав» и датой в .docx-документе.
    # Если NULL — fallback на event.title (старое поведение).
    # Пример: «1 эшелона аэромобильной группировки ФГКУ «ЦСООР «Лидер»»
    docx_subtitle = Column(Text, nullable=True)
