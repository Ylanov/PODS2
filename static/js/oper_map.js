...
function _initMap() {
    // КРИТИЧЕСКИ ВАЖНО — crs: L.CRS.EPSG3395 (World Mercator, эллипсоидальная).
    // Yandex Maps рендерят тайлы именно в этой проекции; если оставить
    // дефолт Leaflet'а EPSG:3857 (Web Mercator, сферический), то тайлы
    // лягут «правильно» (Leaflet считает x/y по своей формуле и Яндекс
    // отдаёт по той же z/x/y — выглядит OK), а маркеры по WGS84
    // координатам (lat/lng) будут СДВИНУТЫ относительно того, что
    // нарисовано на тайлах. Симптом: точка ставится не туда, но
    // расстояния между точками сохраняются — потому что обе сдвинуты
    // одинаково. Меняем CRS на EPSG3395 — Leaflet проецирует так же
    // как Яндекс, маркеры точно совпадают с местами на карте.
    _map = _L.map('om-map', {
        crs:     _L.CRS.EPSG3395,
