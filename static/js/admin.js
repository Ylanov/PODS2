...
// ─── Управление пользователями (users-v2) ───────────────────────────────────
// Вынесено в отдельный модуль admin/users.js — там вся вкладка целиком:
// permissions-чипы, фильтр, форма создания, модалки прав / модулей / сброса
// пароля. Для совместимости со старыми импортами (auth.js, app.js,
// listenForUpdates) ре-экспортируем публичный API оттуда.
export {
...
// users.js дёргает этот сеттер после loadUsers, чтобы редактор списков
// видел свежий список управлений в выпадайке «Квота».
export function setAvailableDepartments(v) { availableDepartments = v; }
